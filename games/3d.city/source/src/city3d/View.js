...
export class View {
...
}.bind( this ), this.tileSize, this.isWithNormal, this.isWithRoughness, this.isPixelStyle );
 ...
 this.ui.dispose();
		...
		if ( this.command === 'NEW' ) {
		    ...
		    this.ui.add( 'grid', { values: [ 'NEW','HIGH' ], selectable: false, bsize: [ 140, 30 ], spaces: [ 18,2 ], radius: 30 } ).onChange( function ( t ) {

 setTimeout( Main.newMap, 1000, t );
 ...
 option.roughness = 0.2; option.metalness = 0.8;
 ...
 option.roughness = 0.8; option.metalness = 0.2;
 ...
 this.buildingLists[ layer ][ i ][ 5 ] = 1; this.addBaseHouse( ar[ 0 ], ar[ 1 ], ar[ 2 ] );

} else if ( k > 0 && ar[ 5 ] === 1 ) {

 this.buildingLists[ layer ][ i ][ 5 ] = 0; this.removeBaseHouse( ar[ 0 ], ar[ 1 ], ar[ 2 ] );
 ...
 if ( ar[ 3 ] === Base.C[ k ] ) _g = this.pool.geo( 'commercial', k );
 ...
 if ( ar[ 3 ] === Base.I[ k ] ) _g = this.pool.geo( 'industrial', k );
 ...
 if ( ar[ 3 ] === Base.H[ k ] ) _g = this.pool.geo( 'house', k );
    ...
    removeTreePack( ar ) {
    	...
    	while ( i -- ) {
    		...
    		if ( this.tempTreeLayers[ i ] === 1 ) {

 this.rebuildTreeLayer( i );
 ...
 this.land.remove( this.miniTerrain[ e ] );
 ...
 this.removeBaseHouse( ar[ 0 ],ar[ 1 ],ar[ 2 ] );
    ...
    saveCityBuild( saveCity ) {
    	...
    	while ( l -- ) {
    		...
    		saveCity[ l ] = [ 0,0,0 ];
    		if ( this.townLists[ l ] !== undefined ) {

 saveCity[ l ][ 0 ] = this.townLists[ l ];

}

	    	if ( this.houseLists[ l ] !== undefined ) {

 saveCity[ l ][ 1 ] = this.houseLists[ l ];

}

	    	if ( this.buildingLists[ l ] !== undefined ) {

 saveCity[ l ][ 2 ] = this.buildingLists[ l ];
 ...
 this.townLists[ l ] = ldata[ 0 ]; this.rebuildTownLayer( l );

}

	    	if ( ldata[ 1 ] !== 0 ) {

 this.houseLists[ l ] = ldata[ 1 ]; this.rebuildHouseLayer( l );

}

	    	if ( ldata[ 2 ] !== 0 ) {

 this.buildingLists[ l ] = ldata[ 2 ]; this.rebuildBuildingLayer( l );
 ...
 delta = e.detail * 20;
	...
	updateLayer() {
	    ...
	    while ( i -- ) {
	    	...
	    	if ( this.tempHouseLayers[ i ] === 1 ) {

 this.rebuildHouseLayer( i ); this.tempHouseLayers[ i ] = 0;

}

	    	if ( this.tempBuildingLayers[ i ] === 1 ) {

 this.rebuildBuildingLayer( i ); this.tempBuildingLayers[ i ] = 0;
	...
	showPower() {
		...
		while ( i -- ) {
			...
			else if ( powerData[ i ] === 2 ) {

 if ( this.powerMeshs[ i ] == null ) this.addPowerMesh( i, this.findPosition( i ) );

} else if ( powerData[ i ] === 1 ) {

 if ( this.powerMeshs[ i ] !== null ) this.removePowerMesh( i );
